...
# TODO: this test fails on my mac for uint64
@pytest.mark.parametrize("tp", np.sctypes['uint'] + np.sctypes['int'])
...
def test_scale_min_max(tp, mn, mx):
...
def test_array_file_scales(in_type, out_type, err):
    ...
    if not err is None:
        with pytest.raises(err):
            calculate_scale(arr, out_dtype, True)
        return
