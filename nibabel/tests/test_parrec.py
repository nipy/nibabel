...
def test_sorting_multiple_echos_and_contrasts():
    ...
    assert_array_equal(
        vol_labels["image_type_mr"], [0, 0, 0, 1, 1, 1, 2, 2, 2, 3, 3, 3]
    )
...
def test_vol_number():
    ...
    assert_array_equal(
        vol_numbers([1, 3, 0, 3, 1, 0, 3, 1, 0]), [0, 0, 0, 1, 1, 1, 2, 2, 2]
    )
...
def test_vol_is_full():
    ...
    assert_array_equal(
        vol_is_full([3, 2, 1, 2, 3], 3), [True, True, True, False, False]
    )
...
def test_varying_scaling():
    ...
    assert_almost_equal(
        np.reshape(scaled_arr, img.shape, order="F"), img.get_fdata(), 9
    )
    # Check fp scaling
    for i in range(arr.shape[2]):
    ...
    assert_almost_equal(
        np.reshape(scaled_arr, img.shape, order="F"), dv_img.get_fdata(), 9
    )
