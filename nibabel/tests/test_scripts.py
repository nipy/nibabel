...
def check_nib_diff_examples():
    ...
    code, stdout, stderr = run_command(
        ["nib-diff", "--dt", "float64"] + fnames, check_code=False
    )
    for item in checked_fields:
        assert item in stdout
...
@pytest.mark.parametrize(
    ...
    [
        ...
        [
            ["-c", "-s", "--all-counts"],
            "",
            r" \[229725\] \[2, 1.2e\+03\] 2:3 3:2 4:1 5:1.*",
        ],
...
def test_nib_ls_multiple():
    ...
    assert (
        [l[ln : ln + len(i_str)] for l in stdout_lines] == [i_str] * 4
...
def test_parrec2nii_with_data():
    ...
    with InTemporaryDirectory():
        ...
        (
            ...
            ["parrec2nii", "--overwrite", "--dwell-time", dti_par], check_code=False
        )
        assert code == 1
        run_command(
            [
        ...
        assert_almost_equal(
            data[..., np.argsort(DTI_PAR_BVALS, kind="stable")], data_sorted
        )
        del img
