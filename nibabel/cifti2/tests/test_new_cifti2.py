...
def check_geometry_map(mapping):
    ...
    assert (
        mapping.volume.transformation_matrix_voxel_indices_ijk_to_xyz.matrix == affine
    ).all()
...
def create_parcel_map(applies_to_matrix_dimension):
    mapping = ci.Cifti2MatrixIndicesMap(
        applies_to_matrix_dimension, "CIFTI_INDEX_TYPE_PARCELS"
    )
    ...
    mapping.extend(
        [
            ci.Cifti2Surface(
                f"CIFTI_STRUCTURE_CORTEX_{orientation}", number_of_vertices
            )
...
def test_dconn():
    ...
    with InTemporaryDirectory():
        ...
        assert isinstance(img2, ci.Cifti2Image)
        assert_array_equal(img2.get_fdata(), data)
        assert img2.header.matrix.get_index_map(0) == img2.header.matrix.get_index_map(
            1
        )
