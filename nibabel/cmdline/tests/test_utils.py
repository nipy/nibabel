...
def test_get_headers_diff():
    ...
    expected_difference = OrderedDict(
        [
            ...
            (
                'dim_info',
                [np.asarray(0).astype(dtype='uint8'), np.asarray(57).astype(dtype='uint8')],
            ),
            (
                'dim',
                [
            ...
            (
                'xyzt_units',
                [np.array(0).astype(dtype='uint8'), np.array(10).astype(dtype='uint8')],
            ),
            (
                'cal_max',
                [
            ...
            (
                ...
                [
                    ...
                    np.array(
                        [-2.00000000e00, 6.71471565e-19, 9.08102451e-18, 1.17855103e02]
                    ).astype(dtype='float32'),
                ],
            ),
            (
                ...
                [
                    ...
                    np.array(
                        [-6.71471565e-19, 1.97371149e00, -3.55528235e-01, -3.57229424e01]
                    ).astype(dtype='float32'),
                ],
            ),
            (
                ...
                [
                    ...
                    np.array(
                        [8.25548089e-18, 3.23207617e-01, 2.17108178e00, -7.24879837e00]
                    ).astype(dtype='float32'),
                ],
            ),
        ]
...
def test_get_data_diff():
    ...
    # same shape, 2 files
    assert get_data_diff([test_array, test_array_2]) == OrderedDict(
        [('DATA(diff 1:)', [None, OrderedDict([('abs', 1), ('rel', 2.0)])])]
    )

    # same shape, 3 files
    assert get_data_diff([test_array, test_array_2, test_array_3]) == OrderedDict(
        [
            (
                'DATA(diff 1:)',
                [
                    None,
                    OrderedDict([('abs', 1), ('rel', 2.0)]),
                    OrderedDict([('abs', 2), ('rel', 2.0)]),
                ],
            ),
            (
                'DATA(diff 2:)',
                [None, None, OrderedDict([('abs', 1), ('rel', 0.66666666666666663)])],
            ),
        ]
    )
    ...
    assert get_data_diff([test_array_2, test_array_4]) == OrderedDict(
        [('DATA(diff 1:)', [None, {'CMP': 'incompat'}])]
    )

    # different shape, 3 files
    assert get_data_diff([test_array_4, test_array_5, test_array_2]) == OrderedDict(
        [
...
def test_main():
    ...
    expected_difference = OrderedDict(
        [
            ...
            (
                'DATA(md5)',
                ['0a2576dd6badbb25bfb3b12076df986b', 'b0abbc492b4fd533b2c80d82570062cf'],
            ),
        ]
    )
