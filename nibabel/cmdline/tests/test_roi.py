...
def test_lossless_slice_scaling(tmp_path):
    ...
    img = nb.Nifti1Image(
        np.random.uniform(-20000, 20000, (5, 5, 5, 5)), affine=np.eye(4)
    )
    img.header.set_data_dtype("int16")
    img.to_filename(fname)
    img1 = nb.load(fname)
    sliced_fname = tmp_path / "sliced.nii"
    lossless_slice(img1, (slice(None), slice(None), slice(2, 4))).to_filename(
        sliced_fname
    )
    ...
    assert np.array_equal(
        img1.dataobj.get_unscaled()[:, :, 2:4], img2.dataobj.get_unscaled()
    )
...
def test_lossless_slice_noscaling(tmp_path):
    ...
    img = nb.MGHImage(
        np.random.uniform(-20000, 20000, (5, 5, 5, 5)).astype("float32"),
        affine=np.eye(4),
    )
    img.to_filename(fname)
    img1 = nb.load(fname)
...
def test_nib_roi(tmp_path, inplace):
    ...
    retval = main(
        [in_file, out_file, "-i", "1:-1", "-j", "-1:1:-1", "-k", "::", "-t", ":5"]
    )
    assert retval == 0
...
def test_entrypoint(capsys):
    ...
    with mock.patch("sys.argv", ["nib-roi", "--help"]):
        try:
            ...
            assert (
                False
            ), "argparse exits on --help. If changing to another parser, update test."
    captured = capsys.readouterr()
