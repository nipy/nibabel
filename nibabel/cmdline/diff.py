...
def are_values_different(*values):
    ...
    for value in values[1:]:
        if type(value0) != type(
            value
        ):  # if types are different, then we consider them different
            return True
...
def get_headers_diff(file_headers, names=None):
    ...
    for field in fields:
        values = [
            header.get(field) for header in file_headers
...
def get_data_hash_diff(files, dtype=np.float64):
    ...
    md5sums = [
        hashlib.md5(
            np.ascontiguousarray(nib.load(f).get_fdata(dtype=dtype))
        ).hexdigest()
        for f in files
    ]
...
def get_data_diff(files, max_abs=0, max_rel=0, dtype=np.float64):
    ...
    data = [
        f if isinstance(f, np.ndarray) else nib.load(f).get_fdata(dtype=dtype)
        for f in files
    ]
    diffs = OrderedDict()
...
def diff(
    files,
    header_fields="all",
    data_max_abs_diff=None,
    data_max_rel_diff=None,
    dtype=np.float64,
):
