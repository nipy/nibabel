    ...
    with ImageOpener(file_like, "rb") as fobj:
        for i, L in enumerate(SHAPE):
            for j, slicer in enumerate(_slices_for_len(L)):
                ...
                def f():
                    fileslice(
                        fobj, tuple(sliceobj), arr.shape, arr.dtype, offset, order
                    )
...
def bench_fileslice(bytes=True, file_=True, gz=True, bz2=False, zst=True):
    ...
    def my_table(title, times, base):
        ...
        print(
            rst_table(
                times, ROW_NAMES, COL_NAMES, title, val_fmt="{0[0]:3.2f} ({0[1]:3.2f})"
            )
        )
    ...
    if gz:
        ...
        my_table(
            "gz slice - raw (ratio)", np.dstack((gz_times, gz_times / gz_base)), gz_base
        )
    if bz2:
        with InTemporaryDirectory():
