...
from .pydicom_compat import pydicom as pdcm
...
class Nifti1DicomExtension(Nifti1Extension):
    ...
    def __init__(self, code, content, parent_hdr=None):
        """Initialize Nifti1DicomExtension.
        ...
        if isinstance(content, pdcm.dataset.Dataset):
            ...
            raise TypeError(
                "content must be either a bytestring or a pydicom "
...
class Nifti1Extensions(list):
    ...
    def count(self, ecode):
        """Return the number of extensions matching a given *ecode*.
    ...
    @classmethod
    def from_fileobj(cls, fileobj, size, byteswap):
...
class Nifti1Header(SpmAnalyzeHeader):
    ...
    @classmethod
    def from_fileobj(cls, fileobj, endianness=None, check=True):
        raw_str = fileobj.read(cls.template_dtype.itemsize)
        hdr = cls(raw_str, endianness, check)
        ...
        hdr.extensions = cls.exts_klass.from_fileobj(fileobj, extsize, byteswap)
        return hdr
    ...
    @classmethod
    def default_structarr(cls, endianness=None):
        """Create empty header binary block with given endianness."""
        hdr_data = super(Nifti1Header, cls).default_structarr(endianness)
        if cls.is_single:
            hdr_data["magic"] = cls.single_magic
        else:
            hdr_data["magic"] = cls.pair_magic
        return hdr_data

    @classmethod
    def from_header(cls, header=None, check=True):
        ...
        """
        new_hdr = super(Nifti1Header, cls).from_header(header, check)
    ...
    def get_slope_inter(self):
        ...
        if not np.isfinite(inter):
            raise HeaderDataError("Valid slope but invalid intercept {0}".format(inter))
        return slope, inter
    ...
    def get_dim_info(self):
        """Get NIfTI MRI slice etc dimension information.
    ...
    def set_dim_info(self, freq=None, phase=None, slice=None):
        """Set nifti MRI slice etc dimension information.
    ...
    def get_slice_duration(self):
        ...
        if slice_dim is None:
            raise HeaderDataError(
                "Slice dimension must be set " "for duration to be valid"
            )
    ...
    def get_n_slices(self):
        ...
        try:
            ...
            raise HeaderDataError(
                "Slice dimension index (%s) outside "
    ...
    def get_slice_times(self):
        ...
        if slabel == "unknown":
            raise HeaderDataError(
                "Cannot get slice times when " 'Slice code is "unknown"'
            )
        ...
        return (
            (None,) * slice_start + tuple(times) + (None,) * (slice_len - slice_end - 1)
        )
    ...
    def set_slice_times(self, slice_times):
        ...
        if slice_len != len(slice_times):
            raise HeaderDataError(
                "Number of slice times does not " "match number of slices"
            )
        ...
        for time in timed:
            if time is None:
                raise HeaderDataError(
                    "Cannot have None in middle " "of slice time vector"
                )
        ...
        if not np.allclose(np.diff(tdiffs), 0):
            raise HeaderDataError(
                "Slice times not compatible with " "single slice duration"
            )
        ...
        if not matching_labels:
            raise HeaderDataError(
                "slice ordering of %s fits " "with no known scheme" % st_order
            )
    ...
    def _slice_time_order(self, slabel, n_slices):
        ...
        if slabel == "sequential increasing":
            ...
            raise HeaderDataError('We do not handle slice ordering "%s"' % slabel)
    ...
    def _clean_after_mapping(self):
        ...
        """
        self._structarr["magic"] = (
            self.single_magic if self.is_single else self.pair_magic
        )
    ...
    @classmethod
    def _chk_qform_code(cls, hdr, fix=False):
        return cls._chk_xform_code("qform_code", hdr, fix)

    @classmethod
    def _chk_sform_code(cls, hdr, fix=False):
        return cls._chk_xform_code("sform_code", hdr, fix)

    @classmethod
    def _chk_xform_code(cls, code_type, hdr, fix):
        # utility method for sform and qform codes
        rep = Report(HeaderDataError)
        code = int(hdr[code_type])
        recoder = cls._field_recoders[code_type]
    ...
    @classmethod
    def may_contain_header(cls, binaryblock):
        if len(binaryblock) < cls.sizeof_hdr:
            return False

        hdr_struct = np.ndarray(
            shape=(), dtype=header_dtype, buffer=binaryblock[: cls.sizeof_hdr]
        )
...
class Nifti1Pair(analyze.AnalyzeImage):
    ...
    def __init__(self, dataobj, affine, header=None, extra=None, file_map=None):
        super(Nifti1Pair, self).__init__(dataobj, affine, header, extra, file_map)
