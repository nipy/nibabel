...
def generate_data(nb_arrays, common_shape, rng):
    data = [
        rng.rand(*(rng.randint(3, 20),) + common_shape) * 100 for _ in range(nb_arrays)
    ]
    return data
...
class TestArraySequence(unittest.TestCase):
    ...
    def test_creating_arraysequence_from_list(self):
        ...
        for ndim in range(1, N + 1):
            common_shape = tuple(
                [SEQ_DATA["rng"].randint(1, 10) for _ in range(ndim - 1)]
            )
            data = generate_data(
                nb_arrays=5, common_shape=common_shape, rng=SEQ_DATA["rng"]
            )
        ...
        check_arr_seq(
            ArraySequence(iter(SEQ_DATA["data"]), buffer_size), SEQ_DATA["data"]
        )
    ...
    def test_arraysequence_getitem(self):
        ...
        check_arr_seq_view(seq_view, SEQ_DATA["seq"])
        check_arr_seq(
            seq_view, [SEQ_DATA["data"][i] for i, keep in enumerate(selection) if keep]
        )
    ...
    def test_arraysequence_operators(self):
        ...
        def _test_binary(op, arrseq, scalars, seqs, inplace=False):
            ...
            for other in seqs:
                ...
                check_arr_seq(
                    seq, [getattr(e1, op)(e2) for e1, e2 in zip(arrseq, other)]
                )
    ...
    def test_arraysequence_repr(self):
        ...
        nb_arrays = 50
        seq = ArraySequence(
            generate_data(nb_arrays, common_shape=(1,), rng=SEQ_DATA["rng"])
        )
