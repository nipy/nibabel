...
def setup():
    global DATA
    DATA["empty_filenames"] = [
        pjoin(data_path, "empty" + ext) for ext in FORMATS.keys()
    ]
    DATA["simple_filenames"] = [
        pjoin(data_path, "simple" + ext) for ext in FORMATS.keys()
    ]
...
class TestLoadSave(unittest.TestCase):
    ...
    def test_save_from_generator(self):
        ...
        for ext, _ in FORMATS.items():
            ...
            lazy_tractogram = LazyTractogram(
                lambda: filtered, affine_to_rasmm=np.eye(4)
            )
