...
def setup_module():
    ...
    DATA["simple_tractogram"] = Tractogram(
        DATA["streamlines"], affine_to_rasmm=np.eye(4)
    )
...
def check_tractogram_item(
    ...
    for key in data_for_streamline.keys():
        assert_array_equal(
            tractogram_item.data_for_streamline[key], data_for_streamline[key]
        )
...
def check_tractogram(
    ...
    for key in data_per_streamline.keys():
        assert_arrays_equal(
            tractogram.data_per_streamline[key], data_per_streamline[key]
        )
        ...
        for data_dict in lazy_dicts:
            ...
            for k in data_dict.keys():
                if isinstance(data_dict[k], np.ndarray) and np.all(
                    ...
                    assert_array_equal(
                        list(data_dict[k]), list(DATA["data_per_streamline"][k])
                    )
...
class TestTractogram(unittest.TestCase):
    ...
    def test_tractogram_copy(self):
        ...
        assert (
            tractogram.data_per_streamline is not DATA["tractogram"].data_per_streamline
        )
    ...
    def test_creating_invalid_tractogram(self):
        ...
        with pytest.raises(ValueError):
            Tractogram(
                streamlines=DATA["streamlines"], data_per_point={"scalars": scalars}
            )
    ...
    def test_tractogram_to_world(self):
        ...
        transformed_tractogram = tractogram.apply_affine(affine)
        assert_array_equal(
            transformed_tractogram.affine_to_rasmm, np.linalg.inv(affine)
        )
...
class TestLazyTractogram(unittest.TestCase):
    def test_lazy_tractogram_creation(self):
        ...
        with pytest.raises(TypeError):
            LazyTractogram(
                streamlines=DATA["streamlines"], data_per_point=data_per_point
            )

        # Empty `LazyTractogram`
        tractogram = LazyTractogram()
        with pytest.warns(
            Warning, match="Number of streamlines will be determined manually"
        ):
            check_tractogram(tractogram)
        assert tractogram.affine_to_rasmm is None
    ...
    def test_lazy_tractogram_apply_affine(self):
        ...
        assert_array_equal(
            transformed_tractogram._affine_to_apply, np.dot(affine, affine)
        )
        assert_array_equal(
            transformed_tractogram.affine_to_rasmm,
            np.dot(np.eye(4), np.dot(np.linalg.inv(affine), np.linalg.inv(affine))),
    ...
    def test_lazy_tractogram_copy(self):
        ...
        assert (
            tractogram._affine_to_apply is not DATA["lazy_tractogram"]._affine_to_apply
        )
        assert_array_equal(
            tractogram._affine_to_apply, DATA["lazy_tractogram"]._affine_to_apply
        )
